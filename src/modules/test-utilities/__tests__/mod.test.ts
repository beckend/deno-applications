...
import { exists as fsExists } from 'https://deno.land/std@0.99.0/fs/mod.ts'
      ...
      group('deferredPromise', () => {
        test({
          ...
          async fn() {
            ...
            assertEquals(results.message, 'throw')
          },
        })
      })
    })
    ...
    group('utils', () => {
      ...
      group('generateTempFilePath', () => {
        test({
          name: 'works',
          fn() {
            assertEquals(ClassMain.utils.generateTempFilePath().length, 55)
