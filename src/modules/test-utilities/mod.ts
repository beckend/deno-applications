...
import * as mock from 'https://deno.land/x/mock@v0.10.0/mod.ts'
import { join } from 'https://deno.land/std@0.99.0/path/mod.ts'
import {
  generate as generateUUIDV5,
  V5Options,
} from 'https://deno.land/std@0.99.0/uuid/v5.ts'
import { generate as generateUUIDV4 } from 'https://deno.land/std@0.99.0/uuid/v4.ts'
...
> {
  ...
  static utils = {
    ...
    generateTempFilePath(x?: V5Options) {
      return join(
        tempDirectory,
        `${TestUtilities.name}_` +
          generateUUIDV5({
            namespace: x?.namespace ? x.namespace : generateUUIDV4(),
            value: x?.value ? x.value : TestUtilities.name,
            ...x,
          })
      )
    },
    ...
    async executeAndClean<
      TFnExecute extends (x: {
        ...
        readonly generateTempFilePath: (x?: V5Options) => string
        readonly generateUUIDV4: typeof generateUUIDV4
      ...
      try {
        const returned = await fnExecute(
          ObjectFreezeRecursive.deepFreeze({
            fileHandler,
            generateTempFilePath(x) {
              const returnedPathFile =
                TestUtilities.utils.generateTempFilePath(x)
