...
import { EOL } from 'https://deno.land/std@0.99.0/fs/eol.ts'
import { readerFromStreamReader } from 'https://deno.land/std@0.99.0/io/streams.ts'
import {
  parse as yamlParse,
...
import { parse as tomlParse } from 'https://deno.land/std@0.99.0/encoding/toml.ts'
      ...
      const returned = {
        fetch: async () => {
          ...
          if (pathFile) {
            ...
            try {
              await this.DH.dependencies.Deno.copy(reader, file)
            } finally {
              file.close()
            }
      ...
      if (shouldWritePath) {
        ...
        if (contentFile) {
          ...
          if (returnContent && contentBuffer) {
            ...
            try {
              await this.DH.dependencies.Deno.copy(
                await readerSource,
