      ...
      group('checks', () => {
        group('requireRoot', () => {
          test({
            ...
            fn: () =>
              assertRejects(
                () => ClassMain.API.checks.requireRoot(),
                Error,
                'root is required'
              ),
          })
      ...
      group('fs', () => {
        group('exists', () => {
          test({
            ...
            async fn() {
              const resultExists = await ClassMain.API.fs.exists({
                pathFiles: ['/tmp'],
              })

              assertEquals(resultExists, [
                {
                  ...resultExists[0],
                  exists: true,
                  pathFile: '/tmp',
        ...
        group('list', () => {
          ...
          test({
            ...
            async fn() {
              ...
              assertEquals(results.symlink.array.length, 0)
              assertEquals(Object.keys(results.symlink.object).length, 0)
            },
          })
        })
      })
